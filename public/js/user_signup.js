...
function checkvalidation() {
  if (!validatePassword()) {
    document.getElementById("error-span").textContent =
      "Password must be of 8 character with atleast one lowercase , uppercase , number";
    return false;
  }
  if (!validatePhoneNumber()) {
    document.getElementById("error-span").textContent =
      "Phone number must compromise of 10 digits";
    return false;
  }
  if (
...
document.getElementById("form").addEventListener("submit", function (event) {
  ...
  const Signupdata = {
    ...
    gender: gender,
    address: address,
    // Removed imageencoding related to Aadhar card
  };
